#+title: Minimal interactive tutorial to searching in Emacs
#+date: 2016-11-07
#+author: Bastien
#+layout: post
...
Hit =M-e= to edit the search string.  Delete "string" in the minibuffer
...
While in the search loop, hit =M-c= to toggle case sensitivity.
...
Try searching for =maii= instead of =mail= and then hit =DEL= to edit the
search string by deleting the last character.
...
Hit =C-s C-M-i=: it displays a new window with a list of previous
searches.
